...
export class Notification extends Service {
  ...
  static fromJson(json: NotifcationJson) {
    ...
    // @ts-ignore
    for (const key of Object.keys(j)) n[`_${key}`] = j[key];
  ...
  private _parseImageData(imageData?: InstanceType<typeof GLib.Variant>) {
    ...
    const [w, h, rs, alpha, bps, _, data] = imageData // iiibiiay
      .recursiveUnpack<
        [number, number, number, boolean, number, number, GLib.Bytes]
      >();
...
export class Notifications extends Service {
  ...
  Notify(
    ...
    const id = this._notifications.has(replacesId)
      ? replacesId
      : this._idCount++;
    const n = new Notification(
      appName,
  ...
  readonly clear = async () => {
    ...
    return Promise.all(
      this.notifications.map((n, i) => close(n, this.clearDelay * i)),
    );
  };
  ...
  private _cache() {
    ...
    writeFile(JSON.stringify(arr, null, 2), CACHE_FILE).catch((err) =>
      console.error(err),
    );
  }
}
